...
class RouteTest extends TestCase
{
    public function setUp()
    {
        ...
        if (!$this->isVersionAvailable()) {
            $this->markTestSkipped('This feature available for Laravel 5.5 and higher');
        }
    }
    /** @test */
    public function test_role_function()
    {
        ...
        $router->get('role-test', $this->getRouteResponse())->name('role.test')->role('superadmin');
    ...
    {
        $router = $this->getRouter();
        $router->get('permission-test', $this->getRouteResponse())->name('permission.test')->permission(['edit articles', 'save articles']);
    ...
    {
        $router = $this->getRouter();
        $router->get('role-permission-test', $this->getRouteResponse())->name('role-permission.test')->role('superadmin|admin')->permission('create user|edit user');
        $this->assertEquals(['role:superadmin|admin', 'permission:create user|edit user'], $this->getLastRouteMiddlewareFromRouter($router));
    }
